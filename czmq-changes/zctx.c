...
struct _zctx_t {
    ...
    int maxsockets;             //  Number of sockets, default from zeromq
};
...
zctx_shadow (zctx_t *ctx)
{
    ...
    self->maxsockets = ctx->maxsockets;
...
//  --------------------------------------------------------------------------
//  Configure max number of sockets in context.  Call before creating
//  any sockets.

void
zctx_set_maxsockets (zctx_t *self, int maxsockets)
{
    assert (self);
    zmutex_lock (self->mutex);
...
zctx__initialize_underlying (zctx_t *self)
{
    ...
    if (!self->context) {
        ...
        if (self->maxsockets) {
            zmq_ctx_set(self->context, ZMQ_MAX_SOCKETS, self->maxsockets);
        }
    }
    zmutex_unlock (self->mutex);
}
